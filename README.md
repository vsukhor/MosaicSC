...
```console
git clone https://github.com/vsukhor/mosaicsc.git
```

The code requires a C++20 - capable compiler. It was tested on and Ubuntu (gcc-14.1).

Using [cmake](https://cmake.org) (ver. 3.17 or higher):

```console
cd mosaicsc
mkdir build_dir
cmake -S . -B build_dir -DCMAKE_BUILD_TYPE=Release -DMOSAICSC_USE_CUDA=ON
cmake --build build_dir
```
Setting 'MOSAICSC_USE_CUDA' can be optionally used to enable pseudorandom number generator provided by Nvidia Cuda toolkit (default is C++ STL).
